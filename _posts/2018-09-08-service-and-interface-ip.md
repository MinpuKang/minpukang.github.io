...
起因是要升级一台本地的通信网络设备A，结果升级之后收不到远端设备B回复的GTP信令消息了，然后A就把B的IP标记成了不可使用的IP，这也是A设备的一个功能，而且标记为不可用的IP后，当下次用户上线就不选择此IP进行GTP Tunnel的建立了，从而避免了时间和资源的浪费，多美完美。

结果客户炸毛了，质问为什么要标记成不可用，升级前怎么没有，能不能关了？质疑的多了，搞的我都怀疑设备A真的有问题了，但是还是坚信客户传输网络有问题。
...
#### 1.什么是接口地址(Sevice IP)和业务地址(Interface IP)
...
对应到网络中，接口地址就是网络设备接口上配置的IP，是要关联到端口的；业务地址则是真实表示业务的逻辑IP地址比如DNS地址、NTP地址、MME的S6a业务地址、PGW的S5/S8业务地址等等。
...
C这个时候出来了说：“前方路途险峻，女施主就不要去啦，贫僧愿代劳前往。”
...
而当我们在设备B上以业务地址作为源来ping设备A的时候发现业务地址是不同的，从而判断传输网络必然存在问题。
...
那么一个问题来了：有木有业务地址和接口地址是一样的呢？答案是肯定的，比如Linux Server上的业务地址就可以直接使用接口地址，但是通信设备上基本不存在。
