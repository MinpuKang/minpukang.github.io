...
2.然后需要先联网打开这篇教程，再断网，包括有限和无限全部关闭。
...
解决办法就是查看准备阶段的/etc/hosts，保证没有任何的abode内容解析。

2.在安装完毕激活的时候一直出现如下图所示内容，没有任何脱机激活提示：
![提示登录][4]

解决办法就是检查/Library/Application Support/Adobe/下下是否存在SLCache、SLStore、SLStore_v1等文件，如果存在，删除之后再打开photoshop尝试激活：
![删除Abode Crash文件][5]
...
打开X-Force，Product包含了几乎Adobe所有的产品（所以本教材也适用于Adobe 2015的其他系列，如果需要可以留言或者后台私信看我是否有相应的软件^_^……小福利）。
我们现在需要选择的产品是Adobe Photoshop CC，然后点击“Gen. serial”生成序列号既Serial，如下：
![X-Force1][6]

**Note：保持X-Force一直为打开状态一直到脱机激活完毕！**


#### 2、安装Photoshop
上一步已经拿到了破解的钥匙，现在我们需要慢慢的打开破解的大门，一步一步的开启Photoshop这座堡垒。

现在需要打开Photoshop的安装包，点击 _**Adobe Photoshop CC 2105**_ 之后选择 _**Install**_ ，此时面对了第一道选择题，是“安装”还是“试用”？因为我们是要破解，试用算个什么鬼，所以此处果断选择 _**安装**_ ，如下图所示：
![安装选择1][7]

点击“安装”之后出现下图左边的登录提示，然后点击 **“登录”** 尝试联网之后出现右侧提示后选择 **“稍后连接”**：
![安装选择2][8]

之后出现了软件许可协议，通读文章并总结中心思想之后当然是选择相信他呀——**“接受”**，然后出现了右图中需要填写序列号的窗口：
![安装选择3][9]

此时将步骤一中X-Force生成的序列号复制并粘贴到Photoshop对话框中（小提示：选中X-Force中的序列号右键选择copy，然后到photoshop对话框第一个输入框右键选择粘贴）：
![安装选择4][10]

填充完序列号，点击下一步又再次出现了联网失败的记录，不要被他蛊惑，依然保持断网状态选择 **“稍后连接”** 进入到安装的后期阶段，选择安装语言和位置之后点击 **“安装”**：
![安装选择5][11]

点击安装之后由于MAX系统的安全，此时需要对用户进入验证，输入当前用户密码并点击**“好”**来确认之后就进入到自动安装阶段，一直到安装完毕出现右图提示：
![安装选择6][12]
...
点击**“脱机激活”**，进入到请求代码生成页面，并点击 _**“生成请求代码”**_ 生成如右图的界面：
![脱机激活2][14]
...
确保在安装和激活过程中X-Force中的序列号不变，同时X-Force使用请求代码生成的响应代码已经完整填充到了脱机激活对话框之后点击神圣的按钮**“激活”**，从而完成了整个的脱机激活过程：：
![脱机激活6][18]

![脱机激活6][19]

点击启动按钮来体验Photoshop的神奇之处吧，此时可以通过 **“帮助”** 按钮看到 **”更新“** 按钮说明激活成功：
![脱机激活7][20]

此时可以关闭X-Force，但是依然要保证**“断网”**状态。

#### 4、修改/etc/hosts文件
关闭Photoshop，依然保持断网状态，我们知道如果一旦连网，如果再次使用photoshop，那么必然要导致其进行官网同步，那么破解大业就烟消云散了，如何解决？那么就要阻止其在联网状态下去跟官网同步，我们的做法是“瞎指挥”。

什么意思呢？我们知道软件跟官网同步都是通过其内置的url通过DNS解析后发送某些数据到官网，那么我们可以通过篡改其域名解析从而达到瞎指挥的目的。

然而如果自己搭建一个DNS server费时费力还要解决其他使用的问题，怎么办，在DNS解析过程其实软件第一步是要到一个叫做hosts的文件中查询地址的，那么我们就可以将adobe的一系列ULR加入到hosts文件中并将其指挥到一个未知的世界从而完美解决官网同步的问题。

在MAC下hosts文件在/etc下，如果熟悉终端的朋友可以直接通过终端**sudo vi /etc/hosts**来添加，但是为了方便更多的朋友，我们这里依然介绍图像化的方式：

a、在Finder下的**“前往”**选择**“前往文件夹”**，然后填写**“/etc”**，回车进入到**etc**文件夹：
![后续问题1][21]

b、由于权限问题，先将hosts文件复制其他地方比如桌面并打开，追加如下内容（注意是追加，也就是从结尾处添加，以下URL基本不会有落网之鱼）：
...
保存关闭，并将修改后的hosts文件复制粘贴回**“/etc”**文件夹，操作如下：
![后续问题2][22]

更新完成后为了万无一失打开**/etc/hosts**文件再次确认已经添加Adobe的系列URL解析：
![后续问题3][23]

到目前为止Photoshop破解已经完成，并且可以联网进行PS的各种操作，但是需要注意的是不要进行登录，即使你已经注册了Adobe。

然后，为了更好的处理RAW文件，PS CC 2015捆绑的是ACR8.0版本，所以需要对ACR进行在线升级，但是最好不要再对Photoshop升级，保持CC 2015状态，因为一旦升级Photoshop可能回面临被识破的局面，如果升级也不要升级到最新的2017，切记切记。

#### 4、联网更新ACR
破解完成并且更新了**/etc/hosts**文件后，就可以联网使用了，此时最好更新一下ACR软件，打开Photoshop之后，在帮助菜单栏找到更新按钮进行可更新软件检测，如对Photoshop Camera Raw 8进行升级，点击更新按钮后输入密码，点击好坐等更新完毕：
![后续问题4][24]
