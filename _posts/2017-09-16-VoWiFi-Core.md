...
wi-Fi Calling已经有3、4年的基础了，所以技术上也越来越成熟，功能也越来越丰富，最主要的是它已经被越来越多的运营商采纳从而部署，今天就简单的聊一聊Wi-Fi Calling的基本构架和信令流程。

## 1. VoWiFi vs VoIP
说到WiFi的语音，必然要涉及到一直存在VoIP（Voice over Internet Protocol），也就是OTT（On the top）业务，通过使用微信、QQ、Skype、Whatapp等等APP来进行的语音视频等。

那么问题来了，既然已经有了这么多的语音视频软件，为什么还要VoWiFi呢？原因可以总结为以下几点：

- a、VoIP必须要下载APP才可以使用；VoWiFi手机可原生支持。
- b、VoIP的APP必须注册，并且大部分都是互相添加好友之后才可以联系，而且每一个APP都是封闭生态圈的，不同的APP之间目前还不能互通；VoWiFi既然手机原生支持，所以不存在APP的弱点，使用运营商的开户手机卡，互相拨打手机号即可。
- c、VoIP的语音视频质量无法保证，完全依赖当前网络质量；而VoWiFi跟VoLTE一样，运营商提供Qos质量保证，已自己测试体验来说VoWiFi的视频质量完全是高清显示，而且无卡顿。
- d、如果离开WiFi环境下，VoIP也就断了或者需要切换到流量模式下来使用；而VoWiFi可以跟VoLTE实现完美切换，从而可以保证离开WiFi环境下依然保持高效的语音视频聊天。

## 2.Wi-Fi Calling拓扑图
Wi-Fi Calling不管是理解为4G语音的一个补充，还是独立存在的一个新的方案，都有着自己的一套完整方案、拓扑和新节点的引入，拓扑如下：

![Wi-Fi Calling拓扑图][3]

#### 2.1 无线接入
Wi-Fi Calling又称为Untrusted Wi-Fi，因为这套方案的无线接入既可以是运营商自己部署AP（Access Point），也可以是各个wi-Fi公司部署的无线比如上海的ishanghai、花生地铁，或者辽宁的iliaoning，万达商场里的无线，星巴克的无线，更或者就是自己家里办的宽带所使用的无线路由器。

所以Wi-Fi Calling的无线接入就是现在无处不在并且被大家没事儿就问密码的只要可以连接互联网的无线网络。

#### 2.2 新旧核心设备
如2/3G的语音需要MSC、4G需要MME一样，Wi-Fi Calling里也引进了一个新的核心设备-ePDG（Evolved Packet Data Gateway），如同MME的作用一样，是无线测和网络册的连接核心，但是更像2/3G的SGSN，因为也要承载用户面的数据。

然后是鉴权数据所存储的HSS，同时也定义了用户的信息比如IMSI、MSIDN和APN已经Qos的相关数据。

之后就是用于连接ePDG和HSS的一个AAA server（Authentication, Authorization and Accounting），用来作为鉴权的桥梁来翻译HSS提供的数据给ePDG。

再之后就是作为核心网和IMS的衔接点，依然使用4G中的PGW作为桥梁。
...
然后呢Wi-Fi对应内层地址还有一个地址可以称为外层地址，这个就是拓扑图中DHCP分配的地址，当然这个DHCP可以是完全独立的一个DHCP server，也可以是具有DHCP功能无线路由器。

外层地址有两个作用，一个是用于查询ePDG SWu地址的时候跟Public DNS通信，二是用于跟ePDG通信建立SWu IPSec隧道。

## 3. Wi-Fi附着信令
说完拓扑，我们来聊聊信令，首先先来看看信令图，下图中表示来VoWiFi的核心信令，包括SWu的IPsec隧道的建立、用户鉴权和GTP隧道建立这三部分，在此之前还需要一些基本的准备工作，解析来我们间的梳理一下：
...
- 1、UE（User Device即终端）发起 **IKE_SA_INIT MID=00** 的请求到ePDG，用来交互IKE的各种算法。
- 2、ePDG回复 **IKE_SA_INIT MID=00 Reponse** 给UE来确认算法。
- 3、UE发起第一个AUTH请求的 **IKE_AUTH MID=01 Request** 消息，其中包括了终端User-Name中包含IMSI、APN、以及终端请求的内层地址的IP类型/DNS/P-CSCF(optional)，同时还有终端支持的IPsec算法
- 4、ePDG收到UE的请求之后开启鉴权，ePDG向AAA发送 **Diameter-EAP-Request（DER）**。
- 5、AAA收到DER之后，因为本地没有任何的用户数据，因此给HSS发送了一条 **Multimedia-Auth Request（MAR）** 用来请求鉴权向量。
- 6、HSS收到MAR之后检测用户已定义之后回复带有鉴权向量的 **Multimedia-Auth Answer（MAA）** 给AAA。
- 7、AAA收到鉴权向量之后，立即发送一个 **Server-Assignment Request（SAR）** 到HSS来请求用户的数据包括APN、Qos信息等。
- 8、HSS回复带有用户数据的 **Server-Assignment Answer（SAA）给AAA**。
- 9、AAA收到SAR之后给回复 **Diameter-EAP Answer（DEA）** 给ePDG，其中包括鉴权向量，并且EAP-AKA Subtype为AKA-Challenge，Result-Code为Result-Code: DIAMETER_MULTI_ROUND_AUTH。
- 10、到此，ePDG回复 **IKE_AUTH MID=01** 给UE，并且包括鉴权数据以及Challenge的要求。
- 11、UE收到MID=01的回复之后发送 **IKE_AUTH MID=02** 来响应来自网络测的Challenge。
...
- 15、AAA收到SAR之后，却是来UE提供的APN确实在HSS已经定义，发送 **Diameter-EAP Answer（DEA）** 到ePDG，其中包括鉴权成功标签、用户信息IMSI/MSISDN等、APN信息以及对应的Qos，其中**QCI**是**5**。
...
- 19、ePDG收到DNS回复的IP并且确认service正确之后，发送 **Create Session Request** 到PGW开始建立用户的GTP隧道，却**RAT-Type**为**WLAN**。
- 20、PGW收到 **Create Session Request** 之后通过**CCR-I**和**CCA-I**通知PCRF建立用户的IP-CAN-Session。
...
- 25、一切信令完成，本地资源分配完成，PGW回复 **Create Session Response** 给ePDG来完成GTP隧道，同时包括给UE分配的内层地址以及DNS/P-CSCF等信息。
- 26、到此，GTP隧道建立完毕，ePDG回 **IKE_AUTH MID=03** 给UE，并将内层地址、DNS/P-CSCF等信息告知UE。
- 27、到此核心网信令完成，为UE准备的IPsec和GTP隧道建立完成，接下来UE可以发起SIP注册来完成IMS网络的注册，以便可以接打电话。
